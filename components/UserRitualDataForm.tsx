    ...
    return (
        <View style={styles.section}>
            ...
            <View style={styles.marginLeft}>
                <FlexRowTexts gap={10} first={<CustomText > ◦ 나의 {ritualName} 리추얼 :</CustomText>} second={<CustomText >{ritualData.activity}</CustomText>} />
                <CustomText > ◦ 리추얼 달성 : {qty} days</CustomText>
                <FlexRowTexts gap={5} first={<CustomText> ◦ 리추얼 시간 : </CustomText>} second={<CustomText> {ritualData.time
                    ? `${ritualData.time.slice(0, 2)}시 ${ritualData.time.slice(2, 4)}분`
                    : "설정한 데이터 없음"}</CustomText>}
                    // 설정 아이콘 클릭 시 time picker open
                    third=
                    {<TouchableOpacity><IconButton iconType="MaterialIcons" icon="settings" onPress={() => { setModalVisible(true) }} size="small" /></TouchableOpacity>} />
                    ...
                    <View style={styles.modalOverlay}>
                        <View style={[styles.modalContent, { backgroundColor: theme.BACKGROUND }]}>
                            ...
                            <View style={{
                                ...
                                <Dropdown data={type === "morning" ? formattedMorningTimes : formattedNightTimes}
                            ...
                            <View style={{ height: 43 }}>
                                {message.type === "info" && (
                                    <Text style={[styles.message, { marginVertical: 10 }]}>
                                        {message.text}
                                    </Text>
                                )}
                            </View>
            ...
            {message.type === "success" && <Text style={[styles.message, { padding: 5, fontSize: 13, textAlign: "center" }]}>{message.text}</Text>}
        </View>
    )
}
