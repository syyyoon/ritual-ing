...
export async function getScheduledNotificationIds() {
  ...
  const notificationIds = scheduledNotifications.map((notification) => notification.identifier);
...
export const scheduleDailyPushNotification = async (time: string, type: "morning" | "night", activity: string) => {
  ...
  const content = {
    ...
    body:
      type === "morning"
        ? `아직 자고 있나요? ${activity}(으)로 하루를 시작해 몸과 마음을 깨워보세요!`
        : `${activity}(으)로 긍정적 에너지를 채우며 하루를 마무리하세요!`,
